...
with st.echo():
  ...
  net = nn.Sequential(
    OrderedDict([
      ('flatten', FlattenLayer().to(getDevice())),
      ('linear', nn.Linear(num_inputs, num_outputs).to(getDevice()))
    ])
  )
...
with st.echo():
  ...
  loss = nn.CrossEntropyLoss().to(getDevice())
...
# else:
#   train_ch3(net, train_iter, test_iter, loss, num_epochs, batch_size,  None, None, optimizer)
train_ch3(net, train_iter, test_iter, loss, num_epochs, batch_size,  None, None, optimizer, getDevice())
