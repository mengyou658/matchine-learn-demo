...
W_conv1 = weight_variable([5, 5, 1, 32], 'W_conv1')  # patch 5x5, in size 1, out size 32 # 权重
# 接着定义bias，它的大小是32个长度，因此我们传入它的shape为[32]
b_conv1 = bias_variable([32], 'b_conv1')  # 张量
...
with tf.Session() as sess:
    sess.run(init)
    save_path = saver.save(sess, 'cnnsave/save_cnn.ckpt')
